...
export const ScannerCardStream = ({
  ...
  const cards = useMemo(() => {
    // If we only have 1 image, repeat it more often to ensure smooth looping
    const effectiveRepeat = cardImages.length === 1 ? repeat * 5 : repeat;
    const totalCards = cardImages.length * effectiveRepeat;
  ...
  useEffect(() => {
    ...
    // Recalculate width on mount/change
    ...
    const updateCardEffects = () => {
      ...
      cardLine.querySelectorAll(".card-wrapper").forEach((wrapper: any, index) => {
    ...
    const handleMouseDown = (e: MouseEvent | TouchEvent) => { 
        ...
        cardStreamState.current.lastMouseX = 'touches' in e ? e.touches[0].clientX : (e as MouseEvent).clientX;
    };
    const handleMouseMove = (e: MouseEvent | TouchEvent) => { 
        ...
        cardStreamState.current.velocity = delta * 50; // Add some impulse
    ...
    const handleWheel = (e: WheelEvent) => { 
        ...
        cardStreamState.current.direction = cardStreamState.current.velocity > 0 ? 1 : -1;
    };
    ...
    const animate = (currentTime: number) => {
      ...
      if (!isPaused && !cardStreamState.current.isDragging) {
        if (Math.abs(cardStreamState.current.velocity) > cardStreamState.current.minVelocity) {
            // Friction logic
             if (Math.abs(cardStreamState.current.velocity) > initialSpeed) {
                 cardStreamState.current.velocity *= cardStreamState.current.friction;
             } else if (Math.abs(cardStreamState.current.velocity) < initialSpeed) {
                 // Return to base speed
                 cardStreamState.current.velocity = cardStreamState.current.velocity > 0 ? initialSpeed : -initialSpeed;
             }
        } else {
      ...
      // Loop logic
      if (position < -cardLineWidth / 2) cardStreamState.current.position += cardLineWidth / 2;
      else if (position > 0) cardStreamState.current.position -= cardLineWidth / 2;
